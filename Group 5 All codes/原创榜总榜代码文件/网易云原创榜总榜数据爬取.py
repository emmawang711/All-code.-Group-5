...
browserPath = os.path.join(currentDir, 'chrome/chrome.exe')
...
class NeteaseMusicScraper:
            ...
            if response.status_code == 200:
                ...
                if js['code'] == 200:
                    ...
                    if birthday > 0:
                        ...
                        birth_year = 1970 + (birthday // (1000 * 365 * 24 * 3600))
                        data['age'] = max(0, current_year - birth_year)
                    
                    # data['city'] = profile.get('city', '无')
                    city = profile.get('city', '无')
                    # 将城市代码转换为城市名称
                    city_name = PROVINCE_CITY_MAP.get(city, '未知')               
                    
                    data['city'] = city_name

                    data['sign'] = profile.get('signature', '无').replace('\n', ' ').replace(',', '，')
    ...
    def spider_comments(self):
        ...
        page.listen.start('/comments/get?')
        page.get(url)
        page.wait.doc_loaded()
        
        with open('comments_data.csv', 'w', encoding='utf-8-sig') as f:
            # Write headers
            f.write('nickname,user_id,age,gender,city,signature,content,comment_id,like_count,comment_time\n')
            ...
            while True:
                try:
                    ...
                    if not res.get('data', {}).get('comments'):
                        break
                        
                    comments = res['data']['comments']
            
                    print(f'Processing page {page_num}, found {len(comments)} comments')
                    ...
                    # Go to next page
                    next_button = page.ele("xpath://a[text()='下一页']")
                    if not next_button:
                        break
